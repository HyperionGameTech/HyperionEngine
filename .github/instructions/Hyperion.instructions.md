...
- To interact with the reflection system, use `TypeId::ForType<T>()` to get the TypeId of a type, and `HypClass::GetClass(TypeId)` to get the `HypClass`. Use `Field` and `Method` to access fields and methods reflectively.
