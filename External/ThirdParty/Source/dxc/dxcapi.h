...
// CROSS_PLATFORM_UUIDOF above only emits __declspec(uuid(...)), which MSVC and Clang use
// to implement __uuidof(), but plain GCC/MinGW has no such builtin -- MinGW-w64 instead
// resolves __uuidof() to __mingw_uuidof<T>(), which requires an explicit __CRT_UUID_DECL
// specialization per type. Without these, using __uuidof() on any of these interfaces
// (as ShaderCompiler.cpp does to call DxcCreateInstance) fails to link.
