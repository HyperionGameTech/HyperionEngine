...
public:
        ...
        : m_strataBodiesFile(strataBodiesFile)
    {
        ...
        m_analyzer.SetStrataOutputDirectory(strataOutputDirectory);
...
private:
    ...
    Task<void> GenerateOutputFiles()
    {
        ...
        if (m_analyzer.GetStrataOutputDirectory().Any())
        {
            Assert(m_analyzer.GetStrataOutputDirectory().MkDir(), "Failed to create Strata output directory: {}", m_analyzer.GetStrataOutputDirectory());

            StrataModuleGenerator strataModuleGenerator;

            FileByteWriter strataWriter(m_analyzer.GetStrataOutputDirectory() / "Engine.strata");

            if (!strataWriter.IsOpen())
            {
                m_analyzer.AddError(HYP_MAKE_ERROR(AnalyzerError, "Failed to open Strata output file: {}", {}, -1, m_analyzer.GetStrataOutputDirectory() / "Engine.strata"));
            }
            else
            {
                strataWriter.WriteString("// Auto-generated by Hyperion CodeGen. Do not edit.\n");

                const Set<String> allHandleNames = strataModuleGenerator.CollectHandleNames(m_analyzer);

                // Deterministic ordering for stable output.
                Array<String> sortedHandleNames;

                for (const String& handleName : allHandleNames)
                {
                ...
                for (Module* mod : sortedModules)
                {
                    ...
                    {
                        strataWriter.Write(writer.GetBuffer());
                    }
                }
