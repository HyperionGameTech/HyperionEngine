...
/*! All knobs for procedural terrain generation.
 *
 *  Heights are in world units (meters). Frequencies are in cycles per meter.
 *  The seed mirrors WorldGridLayerInfo::seed - TerrainWorldGridLayer keeps them in sync. */
HYP_STRUCT()
struct TerrainGenerationParams
{
    ...
    float baseAmplitude = 14.0f;

    HYP_FIELD(Property = "BaseFrequency")
    float baseFrequency = 1.0f / 180.0f;

    HYP_FIELD(Property = "BaseOctaves")
    uint32 baseOctaves = 5;

    // mountain regions
    HYP_FIELD(Property = "MountainRegionFrequency")
    float mountainRegionFrequency = 1.0f / 1400.0f;
    ...
    float mountainAmplitude = 140.0f;

    HYP_FIELD(Property = "MountainFrequency")
    float mountainFrequency = 1.0f / 320.0f;
...
public:
    ...
    {
        return m_params.baseAmplitude + m_params.mountainAmplitude + m_params.warpStrength * 0.2f;
    }
...
protected:
    ...
    void ApplyThermalErosion(Span<float> heights, uint32 size, uint32 margin, const Vec2f& scaleXZ) const;
    void ApplyHydraulicPasses(Span<float> heights, uint32 size, uint32 margin, const Vec2f& scaleXZ) const;
