...
{
    return MergePackage_Internal(sourcePackage, 0);
}

Result AssetPackage::MergePackage_Internal(const Handle<AssetPackage>& sourcePackage, uint32 depth)
{
    HYP_SCOPE;

    HYP_LOG(Assets, Debug, "MergePackage_Internal: Merging '{}' into '{}' at depth {}",
        sourcePackage->GetName(), m_name, depth);

    // Prevent excessive recursion depth
    static constexpr uint32 MAX_MERGE_DEPTH = 100;
    if (depth > MAX_MERGE_DEPTH)
    {
        return HYP_MAKE_ERROR(Error, "Maximum merge depth exceeded - possible circular dependency detected");
    }

    if (!sourcePackage.IsValid())
    {
        return HYP_MAKE_ERROR(Error, "Source package is invalid");
    }

    if (sourcePackage.Get() == this)
    {
        return HYP_MAKE_ERROR(Error, "Cannot merge package with itself");
    }

    // Check for circular dependency - if sourcePackage is an ancestor of this package
    Handle<AssetPackage> currentParent = m_parentPackage.Lock();
    while (currentParent.IsValid())
    {
        if (currentParent.Get() == sourcePackage.Get())
        {
            return HYP_MAKE_ERROR(Error, "Cannot merge package '{}' - it would create a circular dependency", sourcePackage->GetName());
        }
        currentParent = currentParent->GetParentPackage().Lock();
    }

    // Check if this package is an ancestor of sourcePackage
    currentParent = sourcePackage->m_parentPackage.Lock();
    while (currentParent.IsValid())
    {
    ...
    // Merge asset objects
    {
        AssetObjectSet sourceAssetObjects;

        {
            Mutex::Guard sourceGuard(sourcePackage->m_mutex);
            sourceAssetObjects = sourcePackage->m_assetObjects;
        }

        for (const Handle<AssetObject>& sourceAssetObject : sourceAssetObjects)
        {
            if (!sourceAssetObject.IsValid())
            {
                continue;
            }

            // Check if asset with same name already exists
            Name uniqueName = GetUniqueAssetName(sourceAssetObject->GetName());

            // If the name is different, we need to rename the asset
            if (uniqueName != sourceAssetObject->GetName())
            {
                if (Result renameResult = sourceAssetObject->Rename(uniqueName); renameResult.HasError())
                {
                    HYP_LOG(Assets, Error, "Failed to rename asset object '{}' to '{}': {}",
                        sourceAssetObject->GetName(), uniqueName, renameResult.GetError().GetMessage());
                    continue;
                }
            }

            // Remove from source package first
            if (Result removeResult = sourcePackage->RemoveAssetObject(sourceAssetObject); removeResult.HasError())
            {
                HYP_LOG(Assets, Error, "Failed to remove asset object '{}' from source package '{}': {}",
                    sourceAssetObject->GetName(), sourcePackage->GetName(), removeResult.GetError().GetMessage());
                continue;
            }

            // Add to this package
            if (Result addResult = AddAssetObject(sourceAssetObject); addResult.HasError())
            {
                HYP_LOG(Assets, Error, "Failed to add asset object '{}' to package '{}': {}",
                    sourceAssetObject->GetName(), m_name, addResult.GetError().GetMessage());
                continue;
            }

            HYP_LOG(Assets, Debug, "Merged asset object '{}' from package '{}' to package '{}'",
                sourceAssetObject->GetName(), sourcePackage->GetName(), m_name);
        }
    }

    // merge subpackages recursively
    {
        AssetPackageSet sourceSubpackages;

        {
            Mutex::Guard sourceGuard(sourcePackage->m_mutex);
            sourceSubpackages = sourcePackage->m_subpackages;
        }

        for (const Handle<AssetPackage>& sourceSubpackage : sourceSubpackages)
        {
            if (!sourceSubpackage.IsValid())
            {
                continue;
            }

            Handle<AssetPackage> targetSubpackage;
            bool isNewSubpackage = false;

            {
                Mutex::Guard guard(m_mutex);

                auto existingIt = m_subpackages.Find(sourceSubpackage->GetName());

                if (existingIt != m_subpackages.End())
                {
                    // Subpackage with same name exists, merge into it
                    targetSubpackage = *existingIt;
                }
                else
                {
                    // create subpackage
                    targetSubpackage = CreateObject<AssetPackage>(sourceSubpackage->GetName(), sourceSubpackage->GetFlags());
                    targetSubpackage->m_registry = m_registry;
                    targetSubpackage->m_parentPackage = WeakHandleFromThis();
                    targetSubpackage->m_friendlyName = sourceSubpackage->m_friendlyName;
                    targetSubpackage->m_uuid = sourceSubpackage->m_uuid;

                    m_subpackages.Insert(targetSubpackage);
                    isNewSubpackage = true;

                    if (IsInitCalled())
                    {
                        InitObject(targetSubpackage);
                        OnSubpackageAdded(targetSubpackage);
                    }
                }
            }

            // do merge
            if (Result result = targetSubpackage->MergePackage_Internal(sourceSubpackage, depth + 1); result.HasError())
            {
                HYP_LOG(Assets, Error, "Failed to merge subpackage '{}' from package '{}': {}",
                    sourceSubpackage->GetName(), sourcePackage->GetName(), result.GetError().GetMessage());

                // If we created a new subpackage and the merge failed, remove it
                if (isNewSubpackage)
                {
                    Mutex::Guard guard(m_mutex);
                    m_subpackages.Erase(m_subpackages.Find(targetSubpackage->GetName()));

                    if (IsInitCalled())
                    {
                        OnSubpackageRemoved(targetSubpackage);
                    }
                }
            ...
            {
                Mutex::Guard sourceGuard(sourcePackage->m_mutex);
                auto sourceIt = sourcePackage->m_subpackages.Find(sourceSubpackage->GetName());

                if (sourceIt != sourcePackage->m_subpackages.End())
                {
                    sourcePackage->m_subpackages.Erase(sourceIt);

                    if (sourcePackage->IsInitCalled())
                    {
                        sourcePackage->OnSubpackageRemoved(sourceSubpackage);
                    }
                }
            }

            // Note: sourceSubpackage is now empty after merge, no need to update its parent reference
            // The content has been transferred to targetSubpackage which already has the correct parent

            HYP_LOG(Assets, Debug, "Merged subpackage '{}' from package '{}' to package '{}'",
                sourceSubpackage->GetName(), sourcePackage->GetName(), m_name);
        }
    }
...
bool AssetRegistry::AddPackage(Handle<AssetPackage>& package, bool mergeIfExists)
{
    HYP_SCOPE;

    if (!package.IsValid())
    {
        HYP_LOG(Assets, Error, "Cannot add invalid package to registry");
        return false;
    }

    if (!package->GetName().IsValid())
    {
        HYP_LOG(Assets, Error, "Cannot add package with invalid name to registry");
        return false;
    }

    Handle<AssetPackage> existingPackage;

    {
        Mutex::Guard guard(m_mutex);

        auto packageIt = m_packages.Find(package->GetName());

        if (packageIt != m_packages.End())
        {
            existingPackage = *packageIt;

            if (!mergeIfExists)
            {
                HYP_LOG(Assets, Warning, "Package '{}' already exists in registry and mergeIfExists is false", package->GetName());
                return false;
            }
        }
        else
        {
            ...
            // Note: package reference already points to the correct package (no change needed)

            HYP_LOG(Assets, Debug, "Added new package '{}' to registry", package->GetName());
            return true;
        }
    }

    // Merge with existing package
    if (existingPackage.IsValid())
    {
        if (Result mergeResult = existingPackage->MergePackage(package); mergeResult.HasError())
        {
            HYP_LOG(Assets, Error, "Failed to merge package '{}': {}", package->GetName(), mergeResult.GetError().GetMessage());
            return false;
        }

        // Update the input reference to point to the existing package that was merged into
        package = existingPackage;

        HYP_LOG(Assets, Debug, "Successfully merged package '{}' into existing package", package->GetName());
        return true;
    }
