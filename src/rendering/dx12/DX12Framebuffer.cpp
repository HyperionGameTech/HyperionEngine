...
DX12Framebuffer::DX12Framebuffer(Vec2u extent, RenderTargetType renderTargetType, uint32 numViews) // @TODO handle multiview
...
DX12AttachmentRef DX12Framebuffer::AddAttachment(const DX12AttachmentRef& attachment)
{
    Assert(attachment.IsValid() && attachment->GetImage().IsValid());
    ...
    m_attachments.Set(binding, attachment);
