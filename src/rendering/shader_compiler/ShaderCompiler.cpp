...
static String BuildDescriptorTableDefines(
  ...
  for (const DescriptorSetDeclaration &descriptorSetDeclaration :
    ...
    descriptorTableDefines +=
        "#define HYP_DESCRIPTOR_SET_INDEX_" +
        String(descriptorSetDeclaration.name.LookupString()) + " " +
    ...
    for (const Array<DescriptorDeclaration> &descriptorDeclarations :
      ...
      for (const DescriptorDeclaration &descriptorDeclaration :
        ...
        descriptorTableDefines +=
            "\t#define HYP_DESCRIPTOR_INDEX_" +
            String(descriptorSetDeclarationPtr->name.LookupString()) + "_" +
            descriptorDeclaration.name.LookupString() + " " +
            String::ToString(flatIndex) + "\n";
      }
    }
  }
...
static String BuildPreamble(const ShaderProperties &properties) {
  ...
  for (const VertexAttribute::Type attributeType :
       properties.GetRequiredVertexAttributes().BuildAttributes()) {
    preamble += String("#define HYP_ATTRIBUTE_") +
                VertexAttribute::mapping[attributeType].name + "\n";
  }
...
DescriptorUsageSet::BuildDescriptorTableDeclaration() const {
  ...
  for (const DescriptorUsage &descriptorUsage : elements) {
    Assert(descriptorUsage.slot != DescriptorSlot::DESCRIPTOR_SLOT_NONE &&
               descriptorUsage.slot < DescriptorSlot::DESCRIPTOR_SLOT_MAX,
    ...
    if (staticDescriptorSetDeclaration != nullptr) {
      Assert(staticDescriptorSetDeclaration->FindDescriptorDeclaration(
                 descriptorUsage.descriptorName) != nullptr,
      ...
      if (!descriptorSetDeclaration) {
        ...
        newDescriptorSetDeclaration.flags =
            staticDescriptorSetDeclaration->flags |
            DescriptorSetDeclarationFlags::REFERENCE;
...
static bool PreprocessShaderSource(ShaderModuleType type,
                                   ShaderLanguage language, String preamble,
    ...
    const FilePath dir =
        includeDepth > 1
            ...
            : GetResourceDirectory() /
                  FilePath::Relative(basePath, GetResourceDirectory());
...
#define GLSL_ERROR(level, errorMessage, ...)                                   \
  ...
  ByteBuffer shaderModule(glslang_program_SPIRV_get_size(program) *
                          sizeof(uint32));
  glslang_program_SPIRV_get(program,
                            reinterpret_cast<uint32 *>(shaderModule.Data()));
...
struct LoadedSourceFile {
  ...
  FilePath GetOutputFilepath(const FilePath &basePath,
    ...
    return basePath / "data/compiled_shaders/tmp" /
               FilePath(file.path).Basename() +
           "_" + (String::ToString(hc.Value()) + ".spirv");
  }
...
static bool FindVertexAttributeForDefinition(const String &name,
                                             VertexAttribute::Type &outType) {
...
static void
ForEachPermutation(const ShaderProperties &versions,
  ...
  for (const ShaderProperty &valueGroup : valueGroups) {
    // each value group causes the # of combinations to grow by (num values in
    // group) * (current number of combinations)
    Array<ShaderProperties> currentGroupCombinations;
    currentGroupCombinations.Resize(valueGroup.enumValues.Size() *
    ...
    for (SizeType existingCombinationIndex = 0;
      ...
      for (SizeType valueIndex = 0; valueIndex < valueGroup.enumValues.Size();
        ...
        currentGroupCombinations[existingCombinationIndex +
                                 (valueIndex * allCombinations.Size())] =
    ...
    for (const ShaderProperties &properties : currentGroupCombinations) {
      HYP_LOG(ShaderCompiler, Debug, "\t\t{}",
              String::Join(properties, ", ", [](auto &&elem) {
  ...
  HYP_LOG(ShaderCompiler, Info,
          "Processing {} shader permutations:", allCombinations.Size());
  // #endif

  if (parallel) {
    TaskSystem::GetInstance().ParallelForEach(
        allCombinations, [&callback](const ShaderProperties &properties, uint32,
                                     uint32) { callback(properties); });
  } else {
...
static bool LoadBatchFromFile(const FilePath &filepath,
  ...
  if (!compiledShaderBatchOpt.HasValue()) {
    HYP_LOG(ShaderCompiler, Error,
            ...
            filepath, "Failed to deserialize CompiledShaderBatch");
...
String ShaderProperty::GetValueString() const {
  if (HasValue()) {
    ...
    Visit(currentValue,
          [&str](auto &&value) { str = HYP_FORMAT("{}", value); });
...
String ShaderProperties::ToString(bool includeVertexAttributes) const {
  ...
  propertyNames.Reserve(
      (includeVertexAttributes
           ? ByteUtil::BitCount(GetRequiredVertexAttributes().GetFlagMask())
           : 0) +
      GetPropertySet().Size());

  if (includeVertexAttributes) {
    for (Bitset::BitIndex i :
         Bitset(GetRequiredVertexAttributes().GetFlagMask())) {
      AssertDebug(int(i) < int(VertexAttribute::mapping.Size()));
      AssertDebug(VertexAttribute::mapping.ValueAt(i).name != nullptr,
                  "Vertex attribute name missing at index {}, flagmask: {}", i,
                  GetRequiredVertexAttributes().GetFlagMask());

      propertyNames.PushBack(String("HYP_ATTRIBUTE_") +
                             VertexAttribute::mapping.ValueAt(i).name);
    }
  }
...
ShaderCompiler::ShaderCompiler() : m_definitions(nullptr) {
#if HYP_GLSLANG
  ShInitialize();
#endif
}
    ...
    if (sectionIt.first == "permute") {
      ...
      for (const auto &element : sectionIt.second.elements) {
        if (element.subElements.Any()) {
          bundle.versions.AddValueGroup(
              CreateNameFromDynamicString(*element.name), element.subElements);
        } else {
          bundle.versions.AddPermutation(
              CreateNameFromDynamicString(*element.name));
        }
      }
      ...
      bundle.sources[shaderTypeNames.At(sectionIt.first)] =
          SourceFile{GetResourceDirectory() / "shaders" /
                     sectionIt.second.GetValue().name};
    } else {
      HYP_LOG(ShaderCompiler, Warning,
...
bool ShaderCompiler::HandleCompiledShaderBatch(
    Bundle &bundle, const ShaderProperties &requestedProperties,
    const FilePath &outputFilePath, CompiledShaderBatch &batch) {
  ...
  for (auto &sourceFile : bundle.sources) {
    maxSourceFileLastModified =
        MathUtil::Max(maxSourceFileLastModified,
                      FilePath(sourceFile.second.path).LastModifiedTimestamp());
  }
  ...
  ForEachPermutation(
      ...
      [&](const ShaderProperties &properties) {
        ...
        const auto it = batch.compiledShaders.FindIf(
            [propertiesHashCode](const CompiledShader &item) {
              return item.GetDefinition()
                         .GetProperties()
                         .GetPropertySetHashCode() == propertiesHashCode;
            });
  ...
  const bool requestedFound =
      batch.compiledShaders.FindIf(
          [requestedPropertiesHashCode =
               requestedProperties.GetHashCode()](const CompiledShader &item) {
            return item.GetDefinition().GetProperties().GetHashCode() ==
                   requestedPropertiesHashCode;
...

    {
      ...
      for (const ShaderProperties &missingShaderProperties : missingVariants) {
        missingVariantsString +=
            String::ToString(missingShaderProperties.GetHashCode().Value()) +
            " - " + missingShaderProperties.ToString();
...
bool ShaderCompiler::LoadOrCompileBatch(Name name,
                                        const ShaderProperties &properties,
  ...
  const FilePath outputFilePath =
      GetResourceDirectory() / "data/compiled_shaders" / nameString +
      ".hypshader";

  if (outputFilePath.Exists()) {
    HYP_LOG(ShaderCompiler, Info, "Attempting to load compiled shader {}...",
            outputFilePath);
...
bool ShaderCompiler::LoadShaderDefinitions(bool precompileShaders) {
  ...
  TaskSystem::GetInstance().ParallelForEach(bundles, [&](auto &bundle, uint32,
                                                         uint32) {
    if (bundle.HasRTShaders() && !supportsRtShaders) {
      HYP_LOG(ShaderCompiler, Warning,
              "Not compiling shader bundle {} because it contains raytracing "
  ...
  for (const auto &it : results) {
    if (!it.second) {
      ...
      HYP_LOG(ShaderCompiler, Error,
              ...
              it.first->name, it.first->versions.GetHashCode().Value());
...
ShaderCompiler::ProcessResult ShaderCompiler::ProcessShaderSource(
  ...
  if (phase == ProcessShaderSourcePhase::AFTER_PREPROCESS) {
    ...
    result.errors.Concat(
        Map(preprocessErrorMessages, [](const String &errorMessage) {
  ...
  for (uint32 lineIndex = 0; lineIndex < lines.Size();) {
    ...
    case ProcessShaderSourcePhase::BEFORE_PREPROCESS: {
      if (line.StartsWith("HYP_ATTRIBUTE")) {
        ...
        // if (parts.Size() != 3)
        // {
        //     result.errors.PushBack(ProcessError { "Invalid attribute:
        //     Requires format HYP_ATTRIBUTE(location) type name" });
...

        {
          ...
          while (index != parts.Front().Size() &&
                 (std::isalpha(ch = parts.Front()[index]) || ch == '_')) {
          ...
          if (attributeKeyword == "HYP_ATTRIBUTE_OPTIONAL") {
            ...
            result.errors.PushBack(ProcessError{
                String("Invalid attribute, unknown attribute keyword `") +
                attributeKeyword + "`"});

            break;
          }

          if (index != parts.Front().Size() &&
              ((ch = parts.Front()[index]) == '(')) {
            ++index;

            // read integer string
            while (index != parts.Front().Size() &&
                   std::isdigit(ch = parts.Front()[index])) {
        ...
        for (SizeType index = 0;
             index < parts[1].Size() &&
             (std::isalnum(ch = parts[1][index]) || ch == '_');
             index++) {
        ...
        for (SizeType index = 0;
             index < parts[2].Size() &&
             (std::isalnum(ch = parts[2][index]) || ch == '_');
             index++) {
        ...
        if (optional) {
          ...
          if (attributeCondition.HasValue()) {
            result.processedSource += "#if defined(" +
                                      attributeCondition.Get() + ") && " +
                                      attributeCondition.Get() + "\n";
        ...
        result.processedSource +=
            "layout(location=" +
            String::ToString(attributeDefinition.location) + ") in " +
            attributeDefinition.typeClass + " " + attributeDefinition.name +
            ";\n";
    ...
    case ProcessShaderSourcePhase::AFTER_PREPROCESS: {
      if (line.StartsWith("HYP_DESCRIPTOR")) {
        ...
        } else if (commandStr == "HYP_DESCRIPTOR_CBUFF" ||
                   commandStr == "HYP_DESCRIPTOR_CBUFF_DYNAMIC") {
          slot = DESCRIPTOR_SLOT_CBUFF;

          if (commandStr == "HYP_DESCRIPTOR_CBUFF_DYNAMIC") {
            flags |= DescriptorUsageFlags::DYNAMIC;
          }
        } else if (commandStr == "HYP_DESCRIPTOR_SSBO" ||
                   commandStr == "HYP_DESCRIPTOR_SSBO_DYNAMIC") {
        ...
        case DESCRIPTOR_SLOT_SSBO:  // fallthrough
          ...
          result.processedSource +=
              "layout(" + stdVersion + ", set=HYP_DESCRIPTOR_SET_INDEX_" +
              setName + ", binding=HYP_DESCRIPTOR_INDEX_" + setName + "_" +
              descriptorName +
              (additionalParams.Any()
                   ? (", " + String::Join(additionalParams, ", "))
                   : String::empty) +
              ") " + parseResult.remaining + "\n";
          break;
        default:
          result.processedSource +=
              "layout(set=HYP_DESCRIPTOR_SET_INDEX_" + setName +
              ", binding=HYP_DESCRIPTOR_INDEX_" + setName + "_" +
              descriptorName +
              (additionalParams.Any()
                   ? (", " + String::Join(additionalParams, ", "))
                   : String::empty) +
...
bool ShaderCompiler::CompileBundle(Bundle &bundle,
  ...
  for (SizeType index = 0; index < bundle.sources.Size(); index++) {
    ...
    debugName.value =
        ANSIStringView(bundle.sources.AtIndex(index).second.path.Data());

    taskBatch.AddTask([this, index, &bundle, &loadedSourceFiles, &processErrors,
      ...
      if (!reader.IsOpen()) {
        processErrors[index] = {ProcessError{HYP_FORMAT(
            "Failed to open shader source file: {}", std::strerror(errno))}};
  ...
  HYP_LOG(ShaderCompiler, Info, "Compiling shader bundle for shader {}",
          bundle.name);
  ...
  ForEachPermutation(
      ...
      [&](const ShaderProperties &properties) {
        ...
        compiledShader.definition = ShaderDefinition{bundle.name, properties};
        ...
        for (SizeType index = 0; index < loadedSourceFiles.Size(); index++) {
          ...
          const FilePath outputFilepath = item.GetOutputFilepath(
              GetResourceDirectory(), compiledShader.definition);

          filepaths[index] = {outputFilepath, false};

          DescriptorUsageSet &descriptorUsages =
              descriptorUsageSetsPerFile[index];
          ...
          const FilePath dir =
              GetResourceDirectory() /
              FilePath::Relative(FilePath(item.file.path).BasePath(),
                                 GetResourceDirectory());
          ...
          { // Process shader (preprocessing, custom statements, etc.)
            ...
            if (processResult.errors.Any()) {
              ...
              out.errorMessages.Concat(
                  Map(processResult.errors, &ProcessError::errorMessage));
        ...
        for (SizeType index = 0; index < loadedSourceFiles.Size(); index++) {
          ...
          ByteBuffer byteBuffer =
              CompileToSPIRV(item.type, item.language,
                             descriptorUsageSetsMerged, processedSources[index],
                             item.file.path, properties, errorMessages);
        ...
        numCompiledPermutations.Increment(
            uint32(!anyFilesErrored.Get(MemoryOrder::RELAXED) &&
                   anyFilesCompiled.Get(MemoryOrder::RELAXED)),
            MemoryOrder::RELAXED);
        numErroredPermutations.Increment(
            uint32(anyFilesErrored.Get(MemoryOrder::RELAXED)),
            MemoryOrder::RELAXED);

        compiledShader.descriptorTableDeclaration =
            compiledShader.descriptorUsageSet.BuildDescriptorTableDeclaration();
  ...
  if (numErroredPermutations.Get(MemoryOrder::RELAXED)) {
    HYP_LOG(ShaderCompiler, Error,
            ...
            numErroredPermutations.Get(MemoryOrder::RELAXED), bundle.name);

    return false;
  }

  const FilePath finalOutputPath = GetResourceDirectory() /
                                       "data/compiled_shaders" /
                                       String(bundle.name.LookupString()) +
                                   ".hypshader";
  ...
  if (numCompiledPermutations.Get(MemoryOrder::RELAXED) != 0) {
    HYP_LOG(ShaderCompiler, Info,
            ...
            numCompiledPermutations.Get(MemoryOrder::RELAXED), bundle.name,
            finalOutputPath);
  }
...
bool ShaderCompiler::GetCompiledShader(Name name,
                                       const ShaderProperties &properties,
                                       CompiledShader &out) {
  ShaderProperties finalProperties;
  ...
  auto it = batch.compiledShaders.FindIf(
      [finalPropertiesHash](const CompiledShader &compiledShader) {
        if (!compiledShader.IsValid()) {
          return false;
        }

        return compiledShader.GetDefinition().GetProperties().GetHashCode() ==
               finalPropertiesHash;
      });

  if (it == batch.compiledShaders.End()) {
    HYP_LOG(ShaderCompiler, Error,
            "Hash calculation for shader {} does not match {}! Invalid shader "
            "property combination.\n\tRequested instance with properties: "
            "[{}]\n\tFound property sets: {}",
            name, finalPropertiesHash.Value(), finalProperties.ToString(),
            String::Join(batch.compiledShaders, "\n\t", [](auto &&item) {
              return HYP_FORMAT(
                  "Props: {}, Hash: {}",
                  item.GetDefinition().GetProperties().ToString(),
                  item.GetDefinition().GetProperties().GetHashCode().Value());
            }));
